    ...
    const cargarDatosPelicula = async () => {
      try {
        const response = await axios.get(
          `http://localhost:8080/Api/Peliculas/${paramId}`
        );
        const pelicula = response.data;
        console.log(pelicula);
        //let arrayCategorias=pelicula.categoria.map

        const categoriaNombres = pelicula.categorias.map((categoria: { nombre: any; }) => categoria.nombre);;
        setpeliculadata({
          id: pelicula.id,
          ...
          categoria: categoriaNombres,
          url: pelicula.trailer,
          imagen: pelicula.imagen,
        });
        console.log(peliculadata.url);
      } catch (error) {
      ...
      <div className="card">
        ...
        <div className="card-body">
          ...
          <ul className="list-group list-group-flush">
            ...
            <li className="list-group-item">
              <strong>Categoría:</strong> {peliculadata.categoria.join(",")}
            </li>
            <li className="list-group-item">
