...
export default function ModificarPelicula() {
    ...
    const cargarDatosPelicula = async () => {
      try {
        const response = await axios.get(`http://localhost:8080/Api/Peliculas/${paramId}`);
        const pelicula = response.data;
        console.log(pelicula.categorias);
        const categoriaID = pelicula.categorias.map((categoria: { id: any; }) => categoria.id);
        ...
        setFormValues({
          ...
          categoria: categoriaID, // Usar ids de categoría
    ...
    function modificarPeliculaApi(event: React.FormEvent<HTMLFormElement>) {
        ...
        let ruta = `http://localhost:8080/Api/Peliculas/${formValues.id}`;
